...
\begin{frame}{AE1: Penalización vs.\ AE2: Reparación}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{AE1: External Feasibility First}
        \[
          F_{\text{AE1}}(\mathbf{x}) =
          \begin{cases}
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{AE2: Algoritmo de Reparación}
        ...
        \end{enumerate}
        \begin{itemize}
          \item Convergencia más estable desde el inicio.
          \item Reparación aleatoria puede destruir
                buenas estructuras parciales.
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{tabular}{lccc}
        ...
        \midrule
        Pequeña      & \textbf{40.74} & 39.92 & 0.0020* \\
        Mediana      & \textbf{92.14} & 90.38 & 0.0039* \\
        Institucional & \textbf{161.51} & 160.74 & 0.1934 \\
        \bottomrule
      \end{tabular}
      ...
      \begin{tabular}{lcc}
        ...
        \midrule
        AE1 (Penalización) & 3/3 & 2 \\
        AE2 (Reparación)   & 0/3 & 0 \\
        \bottomrule
      \end{tabular}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{alertblock}{Conclusión principal}
        AE1 supera a AE2 en los tres escenarios. Las diferencias
        son significativas en carteras pequeñas y medianas.
        En la institucional, el espacio de búsqueda es tan grande
        que ningún AE llega al óptimo global y la diferencia
        deja de ser significativa.
      \end{alertblock}
      \begin{block}{Interpretación}
        La penalización permite mayor exploración, descubriendo
        combinaciones de acciones descorrelacionadas que la
        reparación aleatoria no alcanza. AE1 sacrifica
        feasibilidad inicial a cambio de mejor calidad final.
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
    ...
    \begin{column}{0.38\textwidth}
      ...
      \begin{itemize}
        \item \textcolor{primaryblue}{\textbf{Azul (AE1):}} arranca
              lento (explora infeasibles), pero converge a
              mayor diversidad final.
        \item \textcolor{accentorange}{\textbf{Naranja (AE2):}}
              muy estable desde el inicio, pero se queda
              en óptimos locales.
        \item La varianza entre runs crece con $n$
              (mayor incertidumbre en espacios grandes).
      \end{itemize}
    \end{column}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.47\textwidth}
      \begin{alertblock}{Limitación fundamental}
        ...
        \vspace{4pt}
        Una cartera muy descorrelacionada puede caer fuerte
        si cada acción reacciona de forma diferente ante
        un shock sistémico (ej.\ COVID-19 en 2020).
      \end{alertblock}

      \vspace{6pt}
      \begin{block}{Hipótesis a validar}
        AE1 (mayor diversidad financiera) $\Rightarrow$
        menor volatilidad y drawdown en el backtest.
      \end{block}
    \end{column}
  \end{columns}
\end{frame}

% --- Backtest - cartera pequeña -------------------------
\begin{frame}{Backtest - Cartera Pequeña ($n=100$, $m=10$)}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \textbf{Observaciones}
      \begin{itemize}
        \item AE1 selecciona acciones muy específicas del
              periodo 2014-2018: sobreajuste pronunciado.
        \item La poca cardinalidad ($m=10$) amplifica la
              volatilidad de cada acción individual.
        \item AE2 actúa aquí como \emph{regularizador}:
              la reparación aleatoria introduce activos que,
              aunque menos descorrelacionados, son más estables.
        \item Los picos de drawdown son similares entre
              ambos enfoques, con ventaja puntual de AE2.
      \end{itemize}
    \end{column}
...
\begin{frame}{Backtest - Cartera Institucional ($n=200$, $m=20$)}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \textbf{Observaciones}
      \begin{itemize}
        \item Con $m=20$ activos la ventaja de AE1 es más
              clara: menor volatilidad rolling consistente.
        \item El drawdown de AE1 es generalmente más
              contenido que el de AE2.
        \item Ambas carteras sufren el shock de COVID-19
              (febrero-marzo 2020), igual que el benchmark.
        \item La diversificación reduce la correlación
              pero no inmuniza ante eventos sistémicos.
      \end{itemize}
    \end{column}
    \begin{column}{0.45\textwidth}
...
\begin{frame}{Conclusiones - Parte 1}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{enumerate}
        \item La \alert{penalización} supera a la \alert{reparación}
              en calidad de solución, confirmado estadísticamente
              en 2 de 3 escenarios.
        \item El elitismo asegura convergencia monótona sin
              sacrificar diversidad poblacional.
        \item En alta dimensión ($n=200$), la ventaja de AE1
              se reduce: el espacio es tan grande que ambos AEs
              quedan lejos del óptimo global.
      \end{enumerate}
    \end{column}
    \begin{column}{0.48\textwidth}
      ...
      \begin{enumerate}
        \item Maximizar descorrelación \emph{no} implica minimizar
              el riesgo de pérdidas absolutas.
        \item En carteras grandes, AE1 produce mejores
              métricas de riesgo en el backtest.
        \item En carteras pequeñas, la reparación aleatoria
              de AE2 actúa como regularizador involuntario.
      \end{enumerate}
      \begin{block}{Trabajo futuro}
        Combinar maximización de diversidad con restricciones
        de rendimiento esperado (Markowitz media-varianza).
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{PSO Continuo (Sección 1)}
        \begin{tabular}{ll}
          \textbf{Espacio} & $\mathbb{R}^3$, continuo \\
